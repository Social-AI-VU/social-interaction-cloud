...
The reference implementation lives in ``sic_framework/mcp/nao_mcp_server.py`` (console entry point ``run-nao-mcp``). Demos that consume it are in ``sic_applications/demos/mcp/``.
...
Some demos (``nao_mcp_voice_client.py`` by default) start the server with **stdio**: the parent launches ``python -m sic_framework.mcp.nao_mcp_server`` and speaks MCP over pipes. **Only stdio supports this spawn model**; HTTP transports expect an already-listening server.
...
Tools are defined in ``nao_mcp_server.py``. Common entries:
...
.. list-table::
   ...
   * - ``sic_applications/demos/mcp/nao_mcp_chat_client.py``
     - Keyboard REPL; default **stdio** subprocess MCP; **--transport sse** for HTTP server in another tab.
   * - ``sic_applications/demos/mcp/nao_mcp_voice_client.py``
     - NAO mic + Google STT; default **stdio** subprocess MCP; **--transport sse** for HTTP server in another tab.
