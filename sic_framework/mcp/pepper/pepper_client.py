"""
Pepper-specific MCP LangChain client configuration and helpers.
"""
...
MCP_SERVER_MODULE = "sic_framework.mcp.pepper.pepper_mcp_server"
MCP_SERVER_NAME = "pepper"
PEPPER_MIC_SAMPLE_RATE_HZ = 16000
...
def build_google_stt_conf(
    *,
    google_keyfile: str,
    language: str = "en-US",
    sample_rate_hertz: int = PEPPER_MIC_SAMPLE_RATE_HZ,
...
def pepper_mcp_session_log_dir(*, caller_file: str) -> str:
...
PEPPER_MCP_CLIENT = McpRobotClientConfig(
    server_name=MCP_SERVER_NAME,
    mcp_server_module=MCP_SERVER_MODULE,
    robot_display_name="Pepper",
    run_server_command="run-pepper-mcp",
    extra_ip_env_vars=("PEPPER_IP",),
)


def resolve_robot_ip(robot_ip: Optional[str] = None) -> Optional[str]:
    return _resolve_robot_ip(
        robot_ip, extra_ip_env_vars=PEPPER_MCP_CLIENT.extra_ip_env_vars
    )
...
def require_robot_ip(robot_ip: Optional[str] = None) -> str:
    return _require_robot_ip(
        robot_ip,
        extra_ip_env_vars=PEPPER_MCP_CLIENT.extra_ip_env_vars,
        robot_display_name=PEPPER_MCP_CLIENT.robot_display_name,
    )


def mcp_sse_connection(*, url: str) -> dict[str, dict[str, Any]]:
    return _mcp_sse_connection(server_name=PEPPER_MCP_CLIENT.server_name, url=url)
...
def print_mcp_stdio_spawn_help(exc: BaseException, *, extra_lines: str = "") -> None:
    _print_mcp_stdio_spawn_help(
        exc, config=PEPPER_MCP_CLIENT, extra_lines=extra_lines
    )
